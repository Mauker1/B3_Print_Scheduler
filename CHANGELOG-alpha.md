...
Twenty-one versions were built before this plugin was published, and every one of them was tested
on a Snapmaker U1 and an Ender 3 rather than on a development machine. Nobody outside this
repository ran any of them, so none of it belongs in the changelog a user reads: that one starts
at the first release. The account is kept here instead, because the reasoning behind a decision
outlives the decision, and most of these entries carry it.

This file is frozen. It is never packaged and never added to: only `plugin/` is built into the
`.b3`, and the manifest points at `plugin/doc/CHANGELOG.md`, which is where every version from
the first release onwards is recorded.
